...
class UpdaterTest {
  ...
  @tailrec
  private def updatedActionRewards(
      averageMethod: (Step) => Double,
      actionRewardsToUpdate: Map[Action, Reward],
      numbers: List[Int],
      action: Action,
      originalSize: Int
  ): Map[Action, Reward] = {
  ...
  def unweightedSampleAverageTest() = {
    ...
    assertEquals(
      expected,
      finalActionRewards(action).toDouble,
      acceptableMargin
    )
  }
  ...
  def exponentialRecencyWeightedAverageTest() = {
    ...
    val finalActionRewards = updatedActionRewards(
      exponentialRecencyWeightedAverage(1),
      actionRewards,
      orderedRewards,
      action,
  ...
  def recencyWeightedAverageTest() = {
    ...
    val finalActionRewards = updatedActionRewards(
      exponentialRecencyWeightedAverage(α),
      actionRewards,
      orderedRewards,
      action,
      orderedRewards.length
  ...
  def stochasticGradientAscentHigherTest() = {
    ...
    val updated = stochasticGradientAscent(α)(
      actionPreferences,
      Action("A"),
      Reward(5),
      Step(1),
    ...
    updated.foreach { (action, preference) =>
