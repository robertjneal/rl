...
val transitionProbabilities = Map(
  (State("A"), Action("0")) -> Map(State("A") -> Probability.unsafe(1), State("B") -> Probability.unsafe(0.0), State("C") -> Probability.unsafe(0.0)),
  (State("A"), Action("1")) -> Map(State("A") -> Probability.unsafe(0.2), State("B") -> Probability.unsafe(0.7), State("C") -> Probability.unsafe(0.1)),
  (State("B"), Action("2")) -> Map(State("A")-> Probability.unsafe(0.0), State("B") -> Probability.unsafe(0.4), State("C") -> Probability.unsafe(0.6)),
  (State("B"), Action("3")) -> Map(State("A") -> Probability.unsafe(0.2), State("B") -> Probability.unsafe(0.6), State("C") -> Probability.unsafe(0.2)),
  (State("C"), Action("4")) -> Map(State("A") -> Probability.unsafe(0.7), State("B") -> Probability.unsafe(0.1), State("C") -> Probability.unsafe(0.2)),
  (State("C"), Action("5")) -> Map(State("A") -> Probability.unsafe(0.3), State("B") -> Probability.unsafe(0.6), State("C") -> Probability.unsafe(0.1))
)

val stateRewards = Map(
  State("A") -> Reward(1),
  State("B") -> Reward(-3),
  State("C") -> Reward(2)
)
...
def equation3dot2(sPrime: State, reward: Reward, currentState: State, action: Action): Probability = {
  if (stateRewards(sPrime) == reward) {
    val stateProbabilities = transitionProbabilities((currentState, action))
...
def equation3dot5(state: State, action: Action): Reward = {
  possibleRewards.fold(Reward(0)){ (expectedReward, r) =>
    expectedReward +
    transitionProbabilities(state, action).keys.foldLeft(Reward(0)) { (reward, sPrime) => Reward(equation3dot2(sPrime, r, state, action).toDouble * r.toDouble) + reward }
  }
}
...
def equation3dot11(γ: Double = 0.9, constant: Double = 0, t: Step = Step(0)): Double = {
  ...
  (for (i <- t.toInt until sequence.length) yield {
    val k = i + 1
    Math.pow(γ, k - (t.toInt) - 1) * sequence(i).toDouble
  }).sum
}
